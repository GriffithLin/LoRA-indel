...
# v：视觉特征，形状为 (batch_size, v_num, h_dim * k)。
# q：问题特征，形状为 (batch_size, q_num, h_dim * k)。
# att_map：注意力权重，形状为 (batch_size, 1, v_num, q_num)
    def attention_pooling(self, v, q, att_map):
        ...
        return fusion_logits #(batch_size, h_dim)

    def forward(self, v, q, softmax=False):
        # v q
        # (batch_size, q_num, q_dim)
        #  v_ q_
        # (batch_size, v_num, h_dim * k)
        # (batch_size, q_num, h_dim * k)
        # att_maps 
        # (batch_size, h_out, v_num, q_num)
        # out
        # (batch_size, h_dim)
